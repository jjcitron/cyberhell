...
def convert_pack1():
    wad_path = 'pack1.wad'
    out_dir = os.path.join('levelPacks', 'pack1')
    ...
    print(f"Found {len(map_indices)} maps in {wad_path}")
    ...
    with open(wad_path, 'rb') as f:
        for map_num, (map_idx, map_name) in enumerate(map_indices, start=1):
            ...
            # Determine default player spawn and ensure player starts on valid floor
            player_spawn = {'pos': [0, 1.2, 0], 'rot': 0}
            for t in things_raw:
                if t[3] == 1: # Player 1 Start
            ...
            map_data = {
                "name": f"Pack 1 - Level {map_num} ({map_name})",
                "skyColor": 526613,
                "fogColor": 724248,
                "fogDensity": 0.015,
            ...
            manifest.append({
                "id": f"json{map_num}",
                "name": map_data["name"],
                "file": f"levelPacks/pack1/{json_filename}",
                "sectors": len(sectors_json),
                "walls": len(walls_json),
                "entities": len(entities_json)
            })

            print(f"Converted Level {map_num} ({map_name}) -> {json_file_path}")

    manifest_path = os.path.join(out_dir, 'manifest.json')
    with open(manifest_path, 'w', encoding='utf-8') as mf:
        json.dump(manifest, mf, indent=2)

    print(f"Successfully generated {len(manifest)} level JSON files and manifest at {manifest_path}")
